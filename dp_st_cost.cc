...
double DpStCost::GetObstacleCost(const StGraphPoint& st_graph_point) {
  ...
  for (const auto* obstacle : obstacles_) {
    ...
    int boundary_index = boundary_map_[boundary.id()];//用哈希表记录障碍物index
    ...
    if (s < s_lower) {
      ...
      const double follow_distance_s = config_.safe_distance();//0.2m

      //与障碍物距离超过0.2m就不计了
      if (s + follow_distance_s < s_lower) {
        continue;
      } else {
        auto s_diff = follow_distance_s - s_lower + s;//这个地方是付的可以理解为越远代价函数越小
        cost += config_.obstacle_weight() * config_.default_obstacle_cost() *
                s_diff * s_diff;//最后的代价函数
      }
    } else if (s > s_upper) {//在下方
      const double overtake_distance_s =
          StGapEstimator::EstimateSafeOvertakingGap();//20m
